...
CodeGenerator
{
  ...
  {
    ps.println(INDENT[INDENT_LEVEL] + access + " String[] " + varName + " = {");

    for(int i = 0; i < data.length; ++i)
    {
      if(i != data.length - 1) {
        ps.println(INDENT[INDENT_LEVEL + 1] + "\"" + data[i] + "\",");
      }
      else {
        ps.println(INDENT[INDENT_LEVEL + 1] + "\"" + data[i] + "\"");
      }
    }
    ps.println(INDENT[INDENT_LEVEL] + "};");
  }
  ...
  {
    ps.println(INDENT[INDENT_LEVEL] + access + " int[] " + varName + " = {");

    ps.print(INDENT[INDENT_LEVEL + 1]);
    for(int i = 0; i < data.length; ++i)
    {
      if(i != 0 && i % 8 == 0) {
  ...
  table(PrintStream ps, int[][] data, String access, String varName)
  {
    ...
    for(int i = 0; i < data.length; ++i)
    {
      ps.println(INDENT[INDENT_LEVEL + 1] + "{");
      ps.print(INDENT[INDENT_LEVEL + 2]);
